function [B,Q,G, orderedEdges, edgesOrder] = parseTopology(netlistFilename)
...
if (isfile(parsingResult))
    ...
    previousData = load(parsingResult, 'B', 'Q', 'G', 'edgesOrder', 'orderedEdges');
    ...
    if numel(order)>0 %isomorphic
        ...
        %Reorder the edges according to previous parsing sort
%          orderedEdges = G.Edges.Variables;
%          temp=sortrows(orderedEdges, 1);
%          orderedEdges(previousData.edgesOrder, :)=temp;
        ...
        edgesOrder = previousData.edgesOrder;
...
if recompute
    ...
    orderedEdges = [cotree.Edges.Variables; tree.Edges.Variables];
    [~, edgesOrder] = sort(orderedEdges(:, 2), 1); %store edgesOrder, not orderedEdges
